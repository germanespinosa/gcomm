...
namespace gcomm {
    ...
    uint16_t GMemConnector::read_buffer(void *buffer, uint16_t max_size) {
        uint16_t pending = _remote->offset >= _remote->next ? _remote->offset - _remote->next : 1024 + _remote->offset -
                                                                                                _remote->next;
        if (pending >= max_size) {
            uint8_t *buffer_c = (uint8_t *) buffer;
            for (int index = 0; index < max_size; index++) {
                buffer_c[index] = _remote->data[_remote->next];
                _remote->next = ( _remote->next + 1 ) % 1024;
            }
            return max_size;
        }
        return 0;
    ...
    void GMemConnector::write_buffer(const void *buffer, uint16_t size) {
        ...
        for (int index = 0; index < size; index++) {
            ...
            _local->offset = ( _local->offset + 1) % 1024;
        }
    }
